...
{	
	...
	outPosition = (root * vec4(v_game_position, 1.0)).xyz;
	gl_Position = PushConstants.camera_matrix * vec4(outPosition, 1.0);
	outNormal = normalize((root * vec4(v_game_normal, 0.0)).xyz);
	outTexCoord = v.tex_coord;
